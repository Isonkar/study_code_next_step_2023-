...
Если нет, она вызывает исходную функцию и сохраняет результат в кеше для использования в будущем. Кеш представляет собой хранилище ранее вычиленных значений, в нашем случае можно использовать словарь

Затем мы применяем этот декоратор к рекурсивной функции fibonacci. Когда функция fibonacci вызывается с определенным входным значением, логика мемоизации проверяет, 
был ли результат уже рассчитан и сохранен в кеше. Если был, кэшированный результат возвращается немедленно. Если нет, то будет вызвана функция fibonacci для вычисления результата, 
...
def memoize(func):
    ...
    @wraps(func)
    def inner(n):
        if n in res_dict.keys(): return res_dict[n]
        else:
            res_dict[n] = func(n)
            return res_dict[n]
    return inner   
